use crate::db::Db;
use crate::modules::session::persistence::SessionRow;
...
impl SessionService {
    ...
    pub fn reset_start_time(&self, now: i64) -> Result<(), String> {
        self.db
            .update_session_started_at(now)
            .map_err(|e| e.to_string())
    }
    ...
    pub fn build_timer_state(&self) -> Result<TimerState, String> {
        ...
        let started_at = if session.state == TrackingState::Active {
